...
def get_center_factor(game, move):
...
def get_moves(game, player):
    ...
    return all_moves, own_moves, opp_moves


def common_moves(game, player):
    _, own_moves, opp_moves = get_moves(game, player)
    intersect_moves = own_moves and opp_moves

    return len(intersect_moves)
...
def interfering_moves(game, player):
    ...
    if not intersect_moves:
        return 0
    return max(get_center_factor(game, m) for m in intersect_moves)
